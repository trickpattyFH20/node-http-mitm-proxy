...
var _sniSpoofer = require('hoxy/lib/sni-spoofer.js');
...
Proxy.prototype.listen = function(options, callback) {
  ...
  this.spoofSNI = !!options.spoofSNI;
    ...
    self.onCertificateRequired(hostname, function (err, files) {
      ...
      async.auto({
        ...
        if (self.forceSNI && !hostname.match(/^[\d\.]+$/)) {
          ...
          if (self.spoofSNI) {
            const spoofer = new _sniSpoofer.SNISpoofer(results.httpsOptions.key, results.httpsOptions.cert);
            const SNICallback = spoofer.callback();
            results.httpsOptions.SNICallback = SNICallback;
          }
          results.httpsOptions.hosts = hosts;
					// add logic: if (args UseGlobalCert) { results.httpsOptions }
					// add spoofer in options here as SNICallback: <func>SNICallback(servername, cb)
...
Proxy.prototype.onCertificateRequired = function (hostname, callback) {
	// TODO check here for global cert
  var self = this;
  return callback(null, {
